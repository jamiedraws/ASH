<%@ Page Language="C#" MasterPageFile="~/VersionViews/Views/Layouts/ConfirmationLayout.master" Inherits="System.Web.Mvc.ViewPage<Dtm.Framework.ClientSites.Web.ClientSiteViewData>" %>
...
%>
	...
	<section class="u-pad @mv-u-pad--bottom clearfix">
        ...
        <%if (!isFraud)
            {
                var orderId = Model.Order.OrderCodes.Where(oc => oc.Label == "DigitalRiverOrderId").Select(oc => oc.Code).FirstOrDefault();                %>
		<!-- // Order Number -->
		<hr><p class="fn--center no-padding"><%= LabelsManager.Labels["ConfirmationOrderNumber"] %> <b class="c-brand__oid"><%= orderId ?? Model.Order.OrderID.ToString() %></b></p><hr>
        <%} %>
