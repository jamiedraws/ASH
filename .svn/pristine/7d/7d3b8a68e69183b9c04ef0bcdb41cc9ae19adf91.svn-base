...
<fieldset class="form__fieldset" id="billingInformation">
    ...
    <ul class="c-brand--form__list">
        ...
        <% if (isProcessPayment) { %>
            <input type="hidden" id="BillingFirstName" name="BillingFirstName" value="<%= DtmContext.Order.BillingFirstName %>" />
            <input type="hidden" id="BillingLastName" name="BillingLastName" value="<%= DtmContext.Order.BillingLastName %>" />    
        <% } %>
    ...
    <%
    bool hasMultipay = false;
    if(DtmContext.Order != null && DtmContext.Order.OrderItems.Any())
    {
        hasMultipay = DtmContext.Order.OrderItems.Any(oi => oi.CampaignProduct.NumberOfPayments > 1);
    }

    if (hasMultipay)
        {
    %>
            <input type="hidden" name="ShippingIsDifferentThanBilling" value="false" />
    <%
        }
        else
        {
    %>
        <label id="ShippingIsSame" for="ShippingIsDifferentThanBilling" class="form__checkbox">
